...
function addInvoiceItemRow(item) {
  ...
  card.innerHTML = `
    ...
    <div class="line-card-body">
      ...
      <div class="line-field">
        <label class="line-label">Description <span class="line-hint">(Enter = new line on PDF)</span></label>
        <textarea class="inv-name line-input line-name-area" rows="2" placeholder="Item name — press Enter to wrap on the PDF" oninput="syncLineCardTitle('${id}'); recalcInvoiceTotal()">${escHtml(nameVal)}</textarea>
      </div>
      <div class="line-field-row">
        <div class="line-field">
          <label class="line-label">Qty</label>
...
/** Wrap PDF text to maxWidth using measured glyph width. Honours Enter / \\n. */
function wrapPdfLines(doc, text, maxWidth) {
  ...
  paragraphs.forEach(function (para) {
    ...
    words.forEach(function (piece) {
      ...
      if (doc.getTextWidth(piece) <= maxW) {
        cur = piece.replace(/^\s+/, '');
        return;
      }
      let chunk = '';
...
function generateInvoicePdf(inv) {
  ...
  loadJsPdf().then(JsPDF => {
    ...
    items.forEach((it) => {
      ...
      // Wrap in Description column only — never spill into Unit / Qty / Amount
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(10.5);
      const nameLines = wrapPdfLines(doc, name || 'Item', descMaxW);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8.5);
      const dLines = desc ? wrapPdfLines(doc, desc, descMaxW).slice(0, 8) : [];
      const nameLineH = 13;
      const descLineH = 11;
...
function generateQuotePdf(q) {
  ...
  loadJsPdf().then(JsPDF => {
    ...
    items.forEach(it => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(10);
      const nl = wrapPdfLines(doc, name, descMaxW);
      const lineH = 12;
